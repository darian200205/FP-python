...
class AppController:
    ...
    def task4(self):
        if not self.memory_representer.get_subject_list():
            ...
            while not self.student_exists(student_to_grade, self.memory_representer.get_student_list()):
                self.ui_representer.show_error()
                student_to_grade = self.ui_representer.get_student_id()

            id_of_subject = self.ui_representer.get_subject_id()
            while not self.subject_exists(id_of_subject, self.memory_representer.get_subject_list()):
                self.ui_representer.show_error()
                id_of_subject = self.ui_representer.get_subject_id()
